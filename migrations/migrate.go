...
func MigrateTables(ctx context.Context) {
	...
	_, err = tx.ExecContext(ctx, users)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, locations)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, terminals)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, routes)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, transactions)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, insertLocations)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, insertTerminals)
	if err != nil {
		tx.Rollback()
		panic(err)
	}

	_, err = tx.ExecContext(ctx, insertRoutes)
	if err != nil {
		tx.Rollback()
		panic(err)
	}
